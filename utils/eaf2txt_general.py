...
# pip.main(["install", 'pympi'])
# import pympi as pmp
# from ../utils/Elan import *
import argparse
import os
import glob
import sys
...
def alternate(file):
    ...
    if input[-4:] == '.eaf':   # A single file has been provided by the user
        eaf2txt(input, output)
    else:                           # A whole folder has been provided
        eaf_files = glob.iglob(os.path.join(input, '*.eaf'))
        for eaf_path in eaf_files:
            print("Processing %s" % eaf_path)
            eaf2txt(eaf_path, output)
